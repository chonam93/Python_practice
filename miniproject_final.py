...
def login():
    while True:
        ...
        if what_to_do == '0':
            ...
            close_conn()
            if result == ():
                print('\n존재하지 않는 회원입니다. 아이디와 비밀번호를 확인해주세요.')
            else:
                print('\n로그인하셨습니다.')
                return ID, PW
...
def delete_cus():
    ...
    while True:
        ...
        if pww == PW:
            connect, curs = connect_curs()
            sql='delete from customer where cus_id = \'{}\''.format(ID)
            curs.execute(sql)
            connect.commit()
            close_conn()
            print('탈퇴가 완료 되었습니다.')
...
def revise_cus():
    global PW
    pww=input('현재 비밀번호를 입력해주세요')
    while True:
        if PW == pww:
            pwnew = input('변경할 비밀번호를 입력해주세요. > ')
            connect, curs = connect_curs()
            sql='update customer set passwd = \'{}\' where cus_id = \'{}\''.format(pwnew, ID)
            curs.execute(sql)
            connect.commit()
            close_conn()
            print('비밀번호가 변경 되었습니다.')
